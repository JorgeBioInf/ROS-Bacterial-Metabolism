...
Disulfide_Bonds.py
----------------------
Computes and retrieves disulfide bonds for each protein in the proteome (including complexes)
...
Usage:
    python Disulfide_Bonds_FULL.py folder_with_predictions
...
# CHECK ARGUMENTS
# A target folder should be entered
...
for subfolder in folder.iterdir():
    ...
    out_filename = f'{subfolder.name}_Disulfide_Bonds.json'
    ...
    full_protein = parser.get_structure("candidate", pdb_path)
    model = full_protein[0]

    for protein,protname in zip(model, complex.split('-')):
        # Check if UniProt info file in subfolder
        data = []
        uniprot_filename = f'{protname}_UniProt_Features.json'
